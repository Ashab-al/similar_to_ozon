...
class Api::Store::Index::SortStoreInteractor < ActiveInteraction::Base
  ...
  def execute
    ...
    return Store.order(asc_or_desc) if asc_or_desc
  ...
  def range_check
    if greater_than 
      errors.add(:greater_than, :invalid) unless greater_than&.is_i? ||  greater_than <= 0
    end

    if less_than
      errors.add(:less_than, :invalid) unless greater_than&.is_i? || greater_than <= 0 || greater_than > less_than
    end
  end
